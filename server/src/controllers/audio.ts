...
export const createAudio  : RequestHandler = async (req: CreateAudioRequest,res:any)=> {
...
export const updateAudio  : RequestHandler = async (req: CreateAudioRequest,res:any)=> {
    const { title, about, category } = req.body;
    const poster = req.files?.poster as formidable.File
    const ownerId  = req.user.id
    ...
    const audio = await Audio.findOneAndUpdate({ _id: audioId},{ title, about, category}, {new: true});
